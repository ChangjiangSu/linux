...
static void adis16480_of_get_ext_clk_pin(struct adis16480 *st,
					 ...
					 enum adis16480_int_pin *pin)
{
	...
	*pin = ADIS16480_PIN_DIO2;
	if (of_property_read_string(of_node, "adi,ext-clk-pin", &ext_clk_pin))
		...
		for (i = 0; i < ARRAY_SIZE(adis16480_int_pin_names); i++) {
			if (strcasecmp(ext_clk_pin,
				       adis16480_int_pin_names[i]) == 0) {
				*pin = i;
				break;
			}
		}
				    ...
				    bool enable)
{
	...
	adis16480_of_get_ext_clk_pin(st, of_node, &pin);
	/*
